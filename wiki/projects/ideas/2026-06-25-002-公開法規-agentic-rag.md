...
description: "可稽核金融法規 RAG；corpus＝AML + 投信利害關係人（2026-06 國泰投信時事錨點）。"
tags: ["side-project", "idea-002", "agent", "金融", "rag", "aml", "投信"]
...
[ETtoday 懶人包](https://finance.ettoday.net/news/3189519)整理國泰投信爭議：董事兼任他公司董事後，**利害關係人名單未及時更新**，基金／全委帳戶仍交易世芯-KY → 出清、通報金管會、基金淨值重算、媒體引述全委損失等。（確定）[[2026-06-25-國泰投信利害關係人-ettoday](/sources/2026-06-25-國泰投信利害關係人-ettoday.md)]

| 媒體敘事關鍵字 | 應對的法遵／法規軸 | Agent 怎麼答 |
|----------------|-------------------|--------------|
| 董事兼任他公司董事 | [證券投資信託基金管理辦法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0400082) 第 10、11 條 利害關係人 | 引用條文定義與交易禁止 |
| 全委帳戶損失 | 全委契約利害關係人限制 + 辦法 | 說**義務與程序**，不預測賠償金額 |
| 通報金管會 | [重大偶發事件通報函](https://law.fsc.gov.tw/LawContent.aspx?id=GL001531) | 列通報義務與事件類型 |
| 內控內稽疏失 | 投信事業管理規則內控 + `aml-bank-ic` 精神 | 原則性引用；不評論個案 |
| 裁罰 300 萬、4.54 億 | **不在 corpus** | **拒答**個案金額／處分 |
...
#002 要解決的**不是**「洗錢法規問答百科」或「新聞摘要機」，而是：

> **用公開法規 corpus，做出可 demo 的金融法規 Agent：法條可引用、個案與裁罰不臆測、golden set 可回歸** — MVP 敘事緊跟 **利害關係人／內控通報** 時事，技術底座仍含 AML。（確定）[[2026-06-25-國泰投信利害關係人-ettoday](/sources/2026-06-25-國泰投信利害關係人-ettoday.md)]

對齊 [年底轉職-agent-準備](/concepts/年底轉職-agent-準備.md) 與 [bloss0m-com](/entities/bloss0m-com.md) 既有 eval 實作（#001 已結案）：**先證明 eval + 安全行為，再談覆蓋多少法規**。（推測）
...
- [ ] 問**投信董事兼任他公司董事** → 引用 [基金管理辦法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0400082) 第 10、11 條（golden 軌 B）
...
以**雙軌公開法規**建 corpus：**B 投信利害關係人＋通報**（時事主軸）、**A 洗錢防制**（銀行合規底座）；驗證可稽核、可拒答、可 eval。
...
| 檔案／模組 | 要寫什麼 |
|------------|----------|
| `corpus/manifest.json` | **6～7 部**法規 metadata；`track`: `aml` \| `sit-related-party` \| `sit-reporting` |
...
| `eval/golden.yaml` | **12 題**分軌 A（AML 5）／B（投信 5）／C（拒答 2） |
...
| 段落 | 要寫什麼 |
|------|----------|
| 問題 | 金融 RAG 為何不能只有「答得像」；以**利害關係人未及時更新**為例（不點名內部、可引公開報導連結） |
...
- [ETtoday](https://finance.ettoday.net/news/3189519) 等**媒體全文**進向量庫（僅 wiki source 歸檔）
...
| ID | 法規／指引 | 來源 | 角色 |
|----|------------|------|------|
| `sit-fund-mgmt` | [證券投資信託基金管理辦法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0400082) | MOJ `G0400082` | **核心**：第 10 條（含不得投資利害關係公司證券）、第 11 條定義 |
| `sit-biz-rules` | [證券投資信託事業管理規則](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0400081) | MOJ `G0400081` | 內部控制、董事監察人變動通報（節錄相關條） |
| `sit-material-event` | [投信投顧重大偶發事件通報](https://law.fsc.gov.tw/LawContent.aspx?id=GL001531) | 金管會函（104.04.14） | 內控缺失、媒體報導等**應立即通報** |

（確定）軌 B 對齊 [[2026-06-25-國泰投信利害關係人-ettoday](/sources/2026-06-25-國泰投信利害關係人-ettoday.md)] 敘事關鍵字。

### 軌 A — 洗錢防制（銀行底座，縮為 2～3 部）

| ID | 法規 | 來源 | 角色 |
|----|------|------|------|
| `aml-finst` | [金融機構防制洗錢辦法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0380252) | MOJ | CDD、監控、申報（與銀行場景連結） |
| `aml-bank-ic` | 銀行業…**內部控制與稽核制度實施辦法** | 金管會／銀行局 | 內控稽核（呼應「內控疏失」敘事） |
| `aml-act` | [洗錢防制法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0380136) | MOJ | **可選**；縮 scope 時刪，保留 `aml-finst` 即可 |

**刪除 MVP 預設：** `aml-act-enf`（施行細則）；與時事無直接關聯者延後。

### 法遵指引（Agent 應輸出的「程序面」checklist）

使用者問「類似媒體這種情況該怎麼做」時，Agent **只列法規義務步驟**（每步附條號），**不**斷言個案是否違法：（推測）

1. **名單更新**：董事／監察人對外兼任變動 → 重新認定利害關係人公司（`sit-fund-mgmt` 第 11 條）
2. **交易管制**：基金不得投資利害關係公司證券；全委依契約停止相關交易（第 10 條 + 契約敘述為原則，不引用內部契約）
3. **庫存處置**：若已違規持有 → 說明辦法上之申報／處置義務（條文 + 「依內控程序」）
4. **通報**：符合重大偶發事件（內控缺失、媒體影響信譽等）→ 立即通報證期局（`sit-material-event`）
5. **受益人權益**：基金淨值重算、補償 → 僅能引用辦法／信託契約**一般義務**，**拒答**具體補償金額
6. **拒答**：特定公司裁罰額、刑事責任、誰賠 4.54 億
...
**為何不是裁罰／ETtoday 進 corpus？** [懶人包](https://finance.ettoday.net/news/3189519) 適合 blog 開場，但 **4.54 億、罰 300 萬**等數字會變，不能做 golden 標準答案。（確定）
...
| 5 | `corpus/README.md` 免責 + **不收录媒體報導**聲明 | 公開 repo 必備 |
...
- **System prompt**：僅 corpus；事實句附 `(證券投資信託基金管理辦法第 X 條)` 等；**禁止**以 ETtoday 等媒體作為法條依據。
...
面試可說：用 [ETtoday 懶人包](https://finance.ettoday.net/news/3189519) 當**需求錨點**，corpus 卻只收 [MOJ／金管會法條](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0400082)；展示 **新聞驅動話題、法條驅動答案、eval 驅動維運** 的分層。（推測）
...
此表評的是 **「這個 side 點子值不值得排進每週至多 1 次的實作」**，不是 RAG 回答品質、也不是 Kaggle／法遵合規分數。（確定）[2026-06-03-年底轉職與時間配置](/sources/2026-06-03-年底轉職與時間配置.md)

| 維度 | 1～5 分在問什麼 |
|------|----------------|
| **求職相關** | 年底轉 Agent 職缺時，面試／履歷能不能當**公開作品**講 |
| **Agent 深度** | 是否展示編排、工具、eval、安全拒答等**企業 Agent**能力（非純 demo） |
| **7 天可交付** | 以 side 時間能否做出**可點驗 MVP**（README + 最小功能） |
| **時事相關** | 能否當 blog／面試的**當下話題**（本案：國泰投信利害關係人爭議） |
| **金融場景** | 是否落在 [金融業-ai-agent-side-發想](/concepts/金融業-ai-agent-side-發想.md) 垂直（發想第五維；舊稱「與 bloss0m 差異化」） |

- **門檻**：總分 **≥16/25** 且 **可完全公開** → 進候選池；與 **#003** 比較排程，每週仍只實作 1 個。
- **技術 eval**（`citation_hit`、`refusal_correct`）見上方「MVP 成功標準」與 golden 12 題 — **另一套分數**。
...
- grounded_in: [2026-06-25-國泰投信利害關係人-ettoday](/sources/2026-06-25-國泰投信利害關係人-ettoday.md)
...
[3] [2026-06-25-國泰投信利害關係人-ettoday](/sources/2026-06-25-國泰投信利害關係人-ettoday.md) — `raw/sources/2026-06-25-國泰投信利害關係人-ettoday.md`
[4] [國泰世華「鐵拳教育」一次看懂](https://finance.ettoday.net/news/3189519) — ETtoday 財經（外部）
[5] [證券投資信託基金管理辦法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0400082) — 全國法規資料庫（外部）
[6] [重大偶發事件通報函](https://law.fsc.gov.tw/LawContent.aspx?id=GL001531) — 金管會（外部）
[7] [金融機構防制洗錢辦法](https://law.moj.gov.tw/LawClass/LawAll.aspx?pcode=G0380252) — 全國法規資料庫（外部）
