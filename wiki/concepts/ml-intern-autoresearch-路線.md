---
type: Concept
title: "ML-Intern × Autoresearch 實驗路線"
description: "ml-intern 與 autoresearch 並行；從 Kaggle 經典競賽切入可重現實驗與自動化研究流程。"
tags: ["ml", "kaggle", "research", "experiment"]
timestamp: 2026-06-25T12:00:00Z
status: active
source_count: 1
---
# ML-Intern × Autoresearch 實驗路線

## Summary

自 **2026-06-25** 起，擁有者認為 [ml-intern-參賽方法](/entities/ml-intern-參賽方法.md) 與 [autoresearch](/entities/autoresearch.md) **值得繼續投入**；入門從 [kaggle](/entities/kaggle.md) **經典競賽**開始，建立 harness、指標與可重跑實驗習慣，支撐轉職與 [bloss0m-com](/entities/bloss0m-com.md) 敘事。（確定）[2026-06-25-學習與-side-主軸調整](/sources/2026-06-25-學習與-side-主軸調整.md)

## Key Points

| 元件 | 角色 |
|------|------|
| [ml-intern-參賽方法](/entities/ml-intern-參賽方法.md) | 可重現 baseline、指標、一鍵重跑（原 AI CUP 脈絡延伸） |
| [autoresearch](/entities/autoresearch.md) | 自動化實驗／研究迭代思路（推測對齊 Karpathy 脈絡） |
| [kaggle](/entities/kaggle.md) | **第一站**：經典競賽作為題目與 leaderboard 基準 |

- 取代 AI CUP 賽季與內容農場，佔用 side **深工**時段。（推測）[2026-06-25-學習與-side-主軸調整](/sources/2026-06-25-學習與-side-主軸調整.md)
- 可與 [#001 Eval Kit](/projects/ideas/2026-06-03-001-agentic-rag-eval-kit.md) 在 **eval 方法論**上呼應。（推測）

## Evidence

- [2026-06-25-學習與-side-主軸調整](/sources/2026-06-25-學習與-side-主軸調整.md)

## Open Questions

- 首個 Kaggle 競賽名稱（Titanic、House Prices、…）？（未知）
- autoresearch 具體指哪個 repo／工具鏈？（未知）
- ml-intern skill 第一個 subcommand 是否改為 `kaggle-submit` 類？（未知）

## Relationships

- related_to: [ml-intern-參賽方法](/entities/ml-intern-參賽方法.md)
- related_to: [autoresearch](/entities/autoresearch.md)
- related_to: [kaggle](/entities/kaggle.md)
- related_to: [年底轉職-agent-準備](/concepts/年底轉職-agent-準備.md)
- related_to: [ai-cup-2026-參賽](/concepts/ai-cup-2026-參賽.md)
- grounded_in: [2026-06-25-學習與-side-主軸調整](/sources/2026-06-25-學習與-side-主軸調整.md)

# Citations

[1] [2026-06-25-學習與-side-主軸調整](/sources/2026-06-25-學習與-side-主軸調整.md) — `raw/sources/2026-06-25-學習與-side-主軸調整.md`
